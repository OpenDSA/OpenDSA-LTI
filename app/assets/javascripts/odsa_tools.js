$(function () {
          ...
          if (result &&
            ...
            Plotly.d3.json("/course_offerings/time_tracking_lookup/" + ODSA_DATA.course_offering_id,
              function (err, data) {
                ...
                var termStartDate = new Date(data["term"][0]["starts_on"] + "T23:59:59-0000")
                var termEndDate = new Date(data["term"][0]["ends_on"] + "T23:59:59-0000")
                var currentDate = new Date()
                if (currentDate <= termStartDate) {
                  reject(`Term starts in a future date ${data["term"][0]["starts_on"]}`)
                  return;
                }
                var trackingEndDate = (termEndDate > currentDate) ? currentDate : termEndDate
                let { weeksDates, daysHash, daysArr } = getWeeksDates(termStartDate, termEndDate)
                ...
                for (var i = 0; i < chapters.length; i++) {
                  ...
                  if (!Object.keys(chaptersModulesHash2).includes(String(ch_id))) {
    ...
    var promise = new Promise((resolve, reject) => {
      odsaStore.keys()
        .then(function (keys) {
          ...
          odsaStore.getItem([dataPrefix, _keys[0]].join('-'))
            .then(function (data) {
      ...
      getStoreData(odsaStore, 'odsaTimeTrackingData')
        .then((result) => {
          if (result &&
            ...
            var date = (result['date']) ? addDay(result['date']) : lookups['term']['starts_on'].replace(/-/g, '')
            ...
            _getTimeTrackingData(odsaStore, '/course_offerings/time_tracking_data/' + ODSA_DATA.course_offering_id + "/date/", date, result['data'], lookups, progressEnd, count)
  ...
  const _getTimeTrackingData = async function (odsaStore, url, date, storeData, lookups, progressEnd, count) {
    var backoff = 1000 * count
    var storeData = storeData || null
    var trackingEndDate = lookups['trackingEndDate']
    var progress = (100 - ((parseInt(trackingEndDate) - parseInt(date)) * 100) / progressEnd)
    ...
    if (parseInt(date) <= parseInt(trackingEndDate)) {
      ...
      return await _getTimeTrackingData(odsaStore, url, addDay(date), storeData, lookups, progressEnd, count)
    } else {
  ...
  function initLinePlot(vizData, lookups) {
    ...
    // selectize code
    // initialize selectize for box plot
    var $selectize_line = $('#select-for-line')
      .selectize({
        plugins: ['remove_button'],
        persist: false,
        maxItems: null,
    ...
    // show current values in multi input dropdown
    $('select#select-for-line.selectized')
      .each(function () {
        var update = function (e) {
          var selectedStudents = $(this).val();
          if (selectedStudents) {
  ...
  function initBarPlot(vizData, lookups) {
    ...
    var promise = new Promise((resolve, reject) => {
      Plotly.newPlot(plotlyBarDiv, plotlyBarData, plotlyBarLayout)
        .then(() => {
          resolve()
        })
    })
    ...
    plotlyBarDiv.on('plotly_buttonclicked', function (e) {
      ...
      if (currentBarTab == 'chapters') {
        $('#sel_modules').hide()
        $('#sel_chapters').show()
        updateBarPlot(selectize_bar_user.items, selectize_bar_ch.items, [], 'chapters')
      } else {
        $('#sel_modules').show()
        $('#sel_chapters').hide()
        updateBarPlot(selectize_bar_user.items, [], selectize_bar_mod.items, 'modules')
      }
    })
    ...
    // selectize code
    // initialize selectize for box plot
    $selectize_bar_user = $('#select-for-bar-user')
      .selectize({
    ...
    $('select#select-for-bar-user.selectized')
      .each(function () {
        var update = function (e) {
          updateBarPlot($(this).val(), selectize_bar_ch.items, selectize_bar_mod.items, currentBarTab)
        }
        $(this).on('change', update);
      });

    var $selectize_bar_ch = $('#select-for-bar-ch')
      .selectize({
        plugins: ['remove_button', 'drag_drop'],
        persist: false,
        maxItems: null,
    ...
    $('select#select-for-bar-ch.selectized')
      .each(function () {
        var update = function (e) {
    ...
    $selectize_bar_mod = $('#select-for-bar-mod')
      .selectize({
        plugins: ['remove_button', 'drag_drop'],
        persist: false,
        maxItems: null,
        ...
        render: {
          item: function (item, escape) {
            return '<div>' +
              '<span>' + escape(item.ch_name) + ' - ' + escape(item.mod_name) + '</span>' +
              '</div>';
          },
          option: function (item, escape) {
            return '<div>' +
    ...
    $('select#select-for-bar-mod.selectized')
      .each(function () {
        var update = function (e) {
          ...
          if (selUsers.length) {
            for (var i = 0; i < selUsers.length; i++) {
              for (var j = 0; j < selChapters.length; j++) {
                traceY.push(chaptersData[chaptersHash[selChapters[j]]][usersHash[selUsers[i]]])
              }
              plotlyBarData.push({
                x: traceX,
                y: traceY,
          ...
          if (selUsers.length) {
            for (var i = 0; i < selUsers.length; i++) {
              for (var j = 0; j < selModules.length; j++) {
                ...
                traceY.push(modulesData[ch_id][modulesHash[selModules[j]]][usersHash[selUsers[i]]])
              }
              plotlyBarData.push({
                x: traceX,
                y: traceY,
  ...
  function initSecBarPlot(vizData, lookups) {
    ...
    var promise = new Promise((resolve, reject) => {
      Plotly.newPlot(plotlySecBarDiv, plotlySecBarData, plotlySecBarLayout)
        .then(() => {
          resolve()
        })
    })
    ...
    $selectize_user_sec = $('#select-for-user-sec')
      .selectize({
        plugins: ['remove_button', 'drag_drop'],
        persist: false,
        maxItems: null,
    ...
    $('select#select-for-user-sec.selectized')
      .each(function () {
        var update = function (e) {
    ...
    var $selectize_mod_sec = $('#select-for-mod-sec')
      .selectize({
        create: true,
        persist: false,
        valueField: 'mod_id',
    ...
    $('select#select-for-mod-sec.selectized')
      .each(function () {
        var update = function (e) {
    ...
    function updateSecBarPlot(selUsers, selModule) {
      ...
      var sectionsNamesArr = (selModule && selChapter) ? sectionsLookup[selChapter][selModule] : []
      var plotlySecBarData = []
      var traceX = []
      var traceY = []
      var barmode = ''
        ...
        if (selUsers.length) {
          for (var i = 0; i < selUsers.length; i++) {
            for (var j = 0; j < sectionsNamesArr.length; j++) {
              traceY.push(sectionsData[selChapter][selModule][j][usersHash[selUsers[i]]])
            }
  ...
  function initViz(vizData, lookups) {
    var promise = new Promise((resolve, reject) => {
      initBoxPlot(vizData, lookups)
        .then(() => {
          initLinePlot(vizData, lookups)
            .then(() => {
              initBarPlot(vizData, lookups)
                .then(() => {
                  initSecBarPlot(vizData, lookups)
                    .then(() => {
                      resolve()
                    })
                })
  ...
  // Starts fetching time tracking data and call initViz function
  function init(count) {
    var count = count || 1
    var promise = new Promise((resolve, reject) => {
      getLookupData(odsaStore)
  ...
  function reloadVizData(count) {
    updateStoreData(odsaStore, 'odsaLoadDataCount', count)
      .then(() => {
        deleteStoreData(odsaStore, 'odsaTimeTrackingData')
          .then(() => {
            deleteStoreData(odsaStore, 'odsaLookupData')
              .then(() => {
                init(count)
                  .then(() => {
                    location.reload();
  ...
  function initExTools(lookups) {
    ...
    var modulesGradableHash = lookups["modulesGradableHash"]

    function clearContainers() {
      $('#multi-container').css('display', 'none');
      $('#single-container').css('display', 'none');
      $('#single-container-exs').css('display', 'none');
    ...
    $('#ex-btn-multi').on('click', function () {
      clearContainers()
      $('#multi-container').css('display', '');
    });

    $('#ex-btn-single').on('click', function () {
      clearContainers()
      $('#single-container').css('display', '');
      $('#single-container-exs').css('display', '');
    });

    //
    // Students selectize code
    //
    ...
    selectize_students = $selectize_students[0].selectize;
    ...
    $('select#select-for-students.selectized')
      .each(function () {
        var update = function (e) {
          ...
          if (user_id) {
            handleSelectStudent(user_id, ODSA_DATA.course_offering_id)
          }
        }
        $(this).on('change', update);
      });
    ...
    //
    // Modules selectize code
    //
    $selectize_modules = $('#select-for-modules')
      .selectize({
        persist: false,
    ...
    selectize_modules = $selectize_modules[0].selectize;
    ...
    //
    // Students exercises selectize code
    //
    $selectize_students_exs = $('#select-for-students-exs')
      .selectize({
        valueField: 'key',
        labelField: 'name',
        searchField: ['name'],
        ...
        render: {
          item: function (item, escape) {
            return (item.attempt_flag) ? `<div><span class="name">${escape(item.name)}</span></div>` : '<div></div>'
    ...
    selectize_students_exs = $selectize_students_exs[0].selectize;

    $('select#select-for-students-exs.selectized')
      .each(function () {
        var update = function (e) {
          var inst_section_id = $(this).val()
          if (inst_section_id) {
            $('#display_table').html('');
            handleDisplay(selectize_students.items, inst_section_id)
          }
        }
        $(this).on('change', update);
      });
    ...
    $('#btn-module-csv').on('click', function () {
      ...
      var fileName = modulesGradableHash[selectize_modules.items]

      var exportName = fileName + '_' + getTimestamp(new Date());
    ...
    function handleDisplay(user_id, inst_section_id) {
      ...
      $.ajax({
        ...
        if (data.odsa_exercise_progress.length == 0 || data.odsa_exercise_attempts.length == 0) {
          ...
          var elem = '<tbody>';
          elem += getFieldMember(data.inst_section,
      ...
      .then((lookups) => {
        getStoreData(odsaStore, 'odsaLoadDataCount')
          .then((result) => {
            ...
            var termStartDate = new Date(lookups["term"]["starts_on"] + "T23:59:59-0000")
            var termEndDate = new Date(lookups["term"]["ends_on"] + "T23:59:59-0000")
            var currentDate = new Date()
            var trackingEndDate = (termEndDate > currentDate) ? currentDate : termEndDate
