...
task :update_module_versions => :environment do
    ...
    def process_reference_config(config_file_path)
        ...
        require 'open3'
        command = ". #{ENV['python_venv_path']} && python3 #{script_path} #{config_file_path} #{output_file_path} --expanded --verbose"
    ...
    def compile_modules(config)
        ...
        require 'open3'
        command = ". #{ENV['python_venv_path']} && python3 #{script_path} #{config_file_path} --standalone-modules -b #{OUTPUT_DIRECTORY_REL}"
        stdout, stderr, status = Open3.capture3(command)
